...
var minimumWhiteTiles = function(floor, numCarpets, carpetLen) {
  ...
  for (let i = 1; i <= n; i++) {
    for (let j = 0; j <= numCarpets; j++) {
      if (j === 0) {
        dp[i][j] = dp[i - 1][j] + Number(floor[i - 1]);
      } else {
        dp[i][j] = Math.min(dp[i - 1][j] + Number(floor[i - 1]), i >= carpetLen ? dp[i - carpetLen][j - 1] : dp[0][j - 1]);
      }
    }
  }
...
console.log(minimumWhiteTiles("10111101", 2, 4))
